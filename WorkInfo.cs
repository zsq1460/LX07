   ...
   public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfo>GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过'order by uploadTime desc limit8";
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from userinfo  workinfo where workVerify='审核通过'and recommmend='是' order by recommendTime desc limit 6";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        ...
        public int Add(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into userinfo workinfo(workId,workName,workPicture,uploadTime,workIntroduction,workVerifty,userName,activityId,recommend,recommendTime)" +
                    "values(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerifty,@userName,@activityId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set  workName=@workName,workPicture=@workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerifty=@workVerifty ," +
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workPicture=@workPicture whrere workId=@workId";
                if (workinfo.workPicture == null)

                    sql = "update workinfo set workIntroduction=@workIntroduction whrere workId=@workId";
        ...
        public int UpdateRecommend(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set recommend=@recommend,recommendTime=@recommendTime whrere workId=@workId";
